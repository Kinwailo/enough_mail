...
class QuotedPrintableMailCodec extends MailCodec {
  ...
  @override
  String decodeText(final String part, final String codec,
      {bool isHeader = false}) {
    final buffer = StringBuffer();
    ...
    for (var i = 0; i < cleaned.length; i++) {
      ...
      if (char == '=' && i + 3 <= cleaned.length) {
        ...
        charCodes.add(char.codeUnitAt(0));
      }
    }
    var decoded = '';
    try {
      decoded = Charset.decode(charCodes, codec);
    } on FormatException catch (err) {
      print('unable to decode quotedPrintable buffer: ${err.message}');
      decoded = String.fromCharCodes(charCodes);
    }
    return decoded;
  }
