  ...
  group('Quoted Printable decoding', () {
    ...
    test('encoding.UTF-8.QuotedPrintable with line break', () {
      const input = 'Viele Gr</span>=C3=BC=C3=9Fe</p=\r\n'
          '>';
      expect(MailCodec.quotedPrintable.decodeText(input, 'utf-8'),
          'Viele Gr</span>üße</p>');
    });

    test('encoding latin1.QuotedPrintable', () {
      const input = 'jeden Tag =E4ndern k=F6nnen';
      expect(MailCodec.quotedPrintable.decodeText(input, 'Latin1'),
    ...
    test(r'encodeText.quoted-printable \r\n line breaks', () {
      ...
      expect(
          MailCodec.quotedPrintable
              .decodeText(MailCodec.quotedPrintable.encodeText(input), 'utf-8'),
          input);
    });
  ...
  group('Q Encoding', () {
    group('Decode examples from https://tools.ietf.org/html/rfc2047#section-8',
      ...
      test('Decode space', () {
        ...
        expect(
            MailCodec.quotedPrintable
                .decodeText(input, 'utf-8', isHeader: true),
            'Keith Moore');
      });
